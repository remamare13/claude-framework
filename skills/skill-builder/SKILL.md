---
name: skill-builder
description: Use when creating new skills, optimizing existing skills, or auditing skill quality. Guides skill development following Claude Code best practices.
user_invocable: true
---

## What This Skill Does

Guides the creation and optimization of Claude Code skills using best practices. Use this whenever:
...
- `user_invocable: true` — Set if the skill can be called via `/name`.
...
- `context: fork` — Set if the skill is self-contained, produces verbose output, and doesn't need conversation history. Runs in a forked context to keep the main conversation clean.
- `agent` — Set alongside `context: fork` to run the skill as a subagent.
- `model` — Set if a specific model capability is needed (e.g., `model: sonnet` for faster/cheaper tasks).
- `allowed-tools` — Restrict which tools the skill can use. Useful for read-only skills or skills that shouldn't edit files.

**Invocation control matrix:**

| `disable-model-invocation` | Result |
|---|---|
| not set (default) | Claude can auto-invoke via natural language AND user can `/name` |
| `true` | User must explicitly `/name` — Claude never auto-triggers |
...
Content rules:
- Keep SKILL.md under 500 lines. Move detailed reference material to supporting files in the same directory.
- Use `$ARGUMENTS` / `$N` for dynamic input from arguments.
- Use `!`command`` for dynamic context injection (runs a command and injects output into the skill).
- Be specific about agent delegation — include exact prompt text.
- Specify all file paths (inputs, outputs, scripts, references).
- Write instructions that a beginner could follow without prior context.
...
.claude/skills/my-skill/
  SKILL.md           ← Main skill (loaded on invocation)
  reference.md       ← Supporting doc (loaded when Claude reads it)
  template.md        ← Template file
  validate.sh        ← Helper script
```

Supporting files are NOT loaded automatically — they load only when Claude reads them via a Read tool call. Reference them from SKILL.md so Claude knows they exist.

**Step 5: Document in CLAUDE.md**

After creating a skill, add a brief entry to the project's `CLAUDE.md`:
...
Not required for the skill to work, but keeps the project organized.
...
| Problem | Solution |
|---------|----------|
| Skill never triggers on natural language | Revise `description` to include keywords people actually say |
| Skill triggers when it shouldn't | Make `description` more specific, add `disable-model-invocation: true` |
| Arguments not substituting | Check `$ARGUMENTS` syntax, ensure `argument-hint` is set |
| Skill output clutters conversation | Add `context: fork` to run in isolated context |
| Skill does too much / accesses wrong tools | Add `allowed-tools` to restrict available tools |
| Supporting files not found | Verify file paths in SKILL.md match actual file locations |
| Skill descriptions not loading | Total description length across all skills may exceed budget — shorten descriptions |

### Complete Example

Here's a minimal but complete skill as a starting template:
...
- [ ] All text is valid UTF-8 (no encoding artifacts or HTML entities)
...
- Always use UTF-8 encoding — no HTML entities for special characters
